...
document.querySelector("#push").onclick = function () {
  ...
  for (let i = 0; i < current_tasks1.length; i++) {
    current_tasks1[i].onclick = function () {
      document.querySelector("#completeTasks").innerHTML +=
        this.closest(".list-group-item").value;
      this.closest(".list-group-item").remove();
    };
  }

  let compTasks = document.getElementById("completedTasks");
  let current_task = document.querySelectorAll(".complButton");
      ...
      document.querySelector("#completedTasks").innerHTML += `
        <li class="list-group-item d-flex w-100 mb-2">
            <div class="w-100 mr-2">
                ...
                <p class="mb-1 w-100">${text.value}</p>
            </div>
            <div class="dropdown m-2 dropleft">
                <button class="btn btn-secondary h-100" type="button" id="dropdownMenuItem1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <i class="fas fa-ellipsis-v"></i>
                ...
                <div class="dropdown-menu p-2 flex-column" aria-labelledby="dropdownMenuItem1">
                    <button type="button" class="btn btn-success w-100 compl">Complete</button>
                    <button type="button" class="btn btn-info w-100 my-2 editButton">Edit</button>
                    <button type="button" class="btn btn-danger w-100 delete">Delete</button>
                </div>
            </div>
